...
const ActivityHistory = (props: any) => {
  ...
  const handleImageModalOk = async () => {
    ...
    const existingActivityIndex = activities.findIndex(
      (activity: any) => activity.monthYear === monthYear
    );
    ...
    if (props?.valueForm?.length > 0) {
      const fetchActivities = async () => {
        const fetchedActivities = await Promise.all(
          props.valueForm.map(async (activity: any) => {
            const images = await Promise.all(
              activity.fileLinks.map(async (link: string) => {
  ...
  return (
    <div className="activity-history">
      <CalendarOutlined
        className="history__calendar-icon"
        onClick={showModal}
      />
      <Modal
        title={props?.label}
          ...
          renderItem={(item: any) => (
            <List.Item
              actions={[
                <span
                  onClick={() =>
                    showImageModalEdit(item?.monthYear, item?.images)
                  }
                  style={{
                    color: "#0073E6",
                    cursor: "pointer",
            ...
            >
              <List.Item.Meta title={item?.monthYear} />
            </List.Item>
          )}
        />
      </Modal>
